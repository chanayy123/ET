...
namespace ETHotfix
{
    ...
    public static class ClientComponentExtensions
    {
        ...
        public static async void InitMatch(this ClientComponent self)
        {
            try
            {
        ...
        public static async ETTask InitClients(this ClientComponent self, int count)
        {
        ...
        public static void StartLogin(this ClientComponent self, int count=0)
        {
            ...
            for(var i = 0; i < count; ++i)
            {
                ...
                if (session == null)
                {
                    ...
                    self.RequestLogin(userId);
                }
            }
        }

        public static async void RequestLogin(this ClientComponent self,int userId)
        {
        ...
        public static void StartMatch(this ClientComponent self, int count = 0)
        {
            Log.Debug($"{count}个测试客户端准备匹配!");
            count = Math.Min(count, self.clientList.Count);
            var serverCfg = StartConfigComponent.Instance.MatchConfig.GetComponent<InnerConfig>();
            for (var i = 0; i < count; ++i)
            {
                var userId = self.clientList[i].UserId;
                self.clientSessionDic.TryGetValue(userId, out Session session);
                if (session == null)
                {
                    session = self.Net.Create(serverCfg.Address2);
                    self.AddSession(userId, session);
                    self.RequestEnterScene(userId);
                }
            }
        }

        public static async void RequestEnterScene(this ClientComponent self,int userId)
        {
        ...
        public static async void TestHttpRequest(this ClientComponent self)
        {
            var httpClient = Game.Scene.AddComponent<HttpClientComponent>();
            try
            {
            ...
            var url = "Login?name=abc&age=123";
            Log.Debug("请求get " + url);
            var res = await httpClient.GetAsync(url);
            Log.Debug("回复get " + res);
            url = "http://192.168.2.128:8080/GetRechargeRecord?id=123";
            Log.Debug("请求get " + url);
            ...
            res = await httpClient.GetAsync(url);
            Log.Debug("回复get " + res);
            url = "http://192.168.2.128:8080/Test1";
