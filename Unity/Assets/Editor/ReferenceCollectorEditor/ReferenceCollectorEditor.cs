...
public class ReferenceCollectorEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        for (int i = referenceCollector.data.Count - 1; i >= 0; i--)
        {
            ...
            var key = EditorGUILayout.TextField(property.stringValue, GUILayout.Width(100));
            property.stringValue = key;
            var goProperty = dataProperty.GetArrayElementAtIndex(i).FindPropertyRelative("gameObject");          
            var type = ETEditor.EditorUIHelper.ObjectToUIType(goProperty.objectReferenceValue);
            type = string.IsNullOrEmpty(type) ? ETEditor.EditorUIHelper.NameToUIType(key) : type;
            property = dataProperty.GetArrayElementAtIndex(i).FindPropertyRelative("type");
            type = string.IsNullOrEmpty(property.stringValue) ? type : property.stringValue;
            type = EditorGUILayout.TextField(type, GUILayout.Width(100));
            property.stringValue = type;
            EditorGUILayout.ObjectField(goProperty.objectReferenceValue, typeof(Object), true);
        ...
        if (eventType == EventType.DragUpdated || eventType == EventType.DragPerform)
        {
            ...
            if (eventType == EventType.DragPerform)
            {
                ...
                {
                    var type = ETEditor.EditorUIHelper.NameToUIType(o.name);
                    AddReference(dataProperty, o.name, type, o);
                }
            }
    ...
    private void AddReference(SerializedProperty dataProperty, string key, string type, Object obj)
    {
