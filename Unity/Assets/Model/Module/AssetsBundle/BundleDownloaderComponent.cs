	...
	public class BundleDownloaderComponent : Component
	{
		...
		public async ETTask StartAsync()
		{
			...
			try
			{
				...
				{
					versionUrl = GlobalConfigComponent.Instance.GlobalConfig.GetUrl() + "StreamingAssets/" + "Version.txt";
				...
				while (true)
				{
					...
					while (true)
					{
						try
						{
							...
							{
								await this.webRequest.DownloadAsync(GlobalConfigComponent.Instance.GlobalConfig.GetUrl() + "StreamingAssets/" + this.downloadingBundle);
